'use client';
import { useState } from 'react';

const YearData = [
  {
    title: '2023-24',
    isOpen: true,
    content: [
    { no: 1, Year: '2023-24', link: '#' }
      ],
  },
  {
    title: '2022-23',
    isOpen: true,
    content: [
      { no: 1, Year: '2022-23', link: '#' }

    ],
    
  },
  {
    title: '2021-22',
  isOpen: true,
    content: [
      { no: 1, Year: '2021-22', link: '#' }
    ]
  },
  {
    title: '2020-21',
    isOpen: true,
    content: [
  { no: 1, Year: '2020-21', link: '#' },
    ],
  },
  {
    title: '2019-20',
   isOpen: true,
    content: [
      { no: 1, Year: '2019-20', link: '#' },
    ]
  },
  {
    title: '2018-19',
    isOpen: true,
    content: [
        { no: 1, Year: '2018-19', link: '#' },
    
    ],
  },
  {
    title: '2017-18',
    isOpen: true,
    content: [
        { no: 1, Year: '2017-18', link: '#' },
    
    ],
   
  },
  {
    title: '2016-17',
   isOpen: true,
    content: [
      { no: 1, Year: '2016-17', link: '#' },
    ],
  },
   {
    title: '2015-16',
   isOpen: true,
    content: [
      { no: 1, Year: '2015-16', link: '#' },
    ],
  },
   {
    title: '2014-15',
   isOpen: true,
    content: [
      { no: 1, Year: '2014-15', link: '#' },
    ],
  },
 
];

export default function Accordion() {
  const [openIndex, setOpenIndex] = useState(0); // First open by default

  const toggle = (index) => {
    setOpenIndex(openIndex === index ? -1 : index);
  };

  return (
    <div className="max-w-4xl mx-auto mt-10 space-y-1">
      {YearData.map((item, index) => (
        <div key={index}>
          <button
            className={`w-full text-left px-4 py-3 font-semibold ${
              openIndex === index ? 'bg-yellow-500' : 'bg-red-800'
            } text-white`}
            onClick={() => toggle(index)}
          >
            {openIndex === index ? '−' : '+'} {item.title}
          </button>

          {openIndex === index && item.content && (
            <div className="border border-t-0 border-gray-300">
              <table className="w-full table-auto text-sm">
                <thead className="bg-red-800 text-white">
                  <tr>
                    <th className="border px-3 py-2">Sr.no</th>
                    <th className="border px-3 py-2">Year</th>
                    <th className="border px-3 py-2">Download</th>
                  </tr>
                </thead>
                <tbody>
                  {item.content.map((row, i) => (
                    <tr key={i} className="text-center">
                      <td className="border px-3 py-2">{row.no}</td>
                      <td className="border px-3 py-2">{row.Year}</td>
                      <td className="border px-3 py-2 text-blue-700 underline cursor-pointer">
                        <a href={row.link} target="_blank" rel="noopener noreferrer">View</a>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          )}
        </div>
      ))}
    </div>
  );
}
